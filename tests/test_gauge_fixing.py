...
class Testgaugefixing(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mc_evaluator = MonteCarloEvaluator(mc_config, system_z2_2_A, gauge_fixing=False)
        self.mc_evaluator_gf = MonteCarloEvaluator(mc_config, system_z2_2_B, gauge_fixing=True)
...
#    @skip("Too long")
    def test_mceval(self):
        ...
        print(f"Energy (no gf): {no_gauge_fixing_energy}, time: {end-start}")
        start = time.time()
        self.mc_evaluator_gf.evaluate()
        gauge_fixing_energy = self.mc_evaluator_gf.get_obs_mean("energy")
        end = time.time()
        print(f"Energy (gf): {gauge_fixing_energy}, time: {end-start}")
