...
def plot(args):
    ...
    if args.ed is not None:
        ...
        for ob in args.obs:
            ...
            yvals = yvals_dict[ob]
            xvals_m, yvals_m = zip(
                *sorted(zip(xvals, yvals))
            )  # we define new variables so as not to change xvals while looping over the observables
    ...
    if args.mc is not None:
        for ind, mc_data in enumerate(args.mc):
            ...
            xvals, yvals_dict, yvals_err_dict = extract_data(
                data, args.xaxis, args.obs, restrictions, args.require_g, get_error=True
            )
            for ob in args.obs:
                ax.errorbar(
                    xvals,
                    yvals_dict[ob],
                    yerr=yvals_err_dict[ob],
                    label=f"MC, obs={ob}",
                    marker="o",
                    linestyle="",
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--restrict",
        nargs="+",
        help="Data restrictions - only plot data that has these couplings. Format: key=val, e.g. mass=1",
    )
    parser.add_argument(
        "--require_g",
        action="store_true",
        default=False,
        help="Restricts data to cases where el and mag have the required relationship (both derived from g)",
    )
