...
def grad_heatmap(X,Y,data,cbar_kw={},cbarlabel="",ax=None, **kwargs):
    ...
    plt.quiver((quiver_x)*x_scale+0.5,(quiver_y)*y_scale,diff_data_x*x_scale,0, color='w')
    #Plot gradient for y
    quiver_x,quiver_y=np.meshgrid(X,Y[:-1])
    plt.quiver((quiver_x)*x_scale,(quiver_y)*y_scale+0.5,0,diff_data_y*y_scale, color='k')
    return im, cbar

if __name__=="__main__":
    X=np.array([0.0,1.0,2.0])
    Y=np.array([0.,2.,4.])
    data=np.array([[3.,4.,7.],[8.,12.,13.],[3.,5.,2.]])
