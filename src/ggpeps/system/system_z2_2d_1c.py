...
class Z2System2D_1c(System2DBase):
    ...
    def update_gauge_ind(self, link_ind: int, theta: float):
        ...
        gamma_neutral_gauge = self.gamma_gauge_neutral_vec[0][
            dir
        ]  # calling every time (rather than storing) will cause some innefficiency
        # just use the first gamma_gauge_neutral, since they're shared by all layers
        ...
        self.gamma_in_sys[ind_mat : ind_mat + 4, ind_mat : ind_mat + 4] = gamma_in_subst
    ...
    def _compute_mass_energy_op_vec_and_grad(self, use_trans_inv=True):
        ...
        raise NotImplementedError(
            "The mass energy is not implemented yet for the 2 copy case."
        )
    ...
    def _compute_int_energy_op_vec_and_grad(self):
        ...
        # We return zeros just to not break the interface.
        raise NotImplementedError(
            "The interaction energy is not implemented yet for the 2 copy case."
        )
