...
def calculate_lognormvec_jax(
    gamma_in_sys_vec: List[np.ndarray], mat_d_vec: List[np.ndarray], all_factors=False
) -> float:
    ...
    if all_factors:
        dest = dest - mat_d_vec[0].shape[0] * np.log(
            2
...
def compute_el_grad_vec_jax(system):
    ...
    dest_grad = []
    el_energy_vec = device_put(
        system.el_energy_op_vec, device=ggpeps.PREFERRED_DEVICE
    ...
    mat_b_vec = (
        system.mat_b_mod_vec
    )  # device_put(jnp.asarray(system.mat_b_mod_vec), device=ggpeps.PREFERRED_DEVICE)
    diff_d_gamma_inv_vec = device_put(
        jnp.asarray(
            [system.wi_gamma_out_mod_vec[layerind].inv() for layerind in layers]
        ),
        device=ggpeps.PREFERRED_DEVICE,
    )  # this does not actually do a computation, just a retrieval
    single_link_offset = 2 * system.cfg.nvirtmodes_link
    offset = 2 * system.cfg.lattice.size + single_link_offset
    overall_factors = jnp.asarray(
        system.cfg.el_overall_factors
    )  # device_put(jnp.asarray(system.cfg.el_overall_factors), device=ggpeps.PREFERRED_DEVICE)
    idxarrs_prefactors = jnp.asarray(
        [[t[0] for t in system.cfg.idxarr_vec[layerind]] for layerind in layers]
    )  # device_put(jnp.asarray([[t[0] for t in system.cfg.idxarr_vec[layerind]] for layerind in layers]), device=ggpeps.PREFERRED_DEVICE)
    idxarrs_indices = jnp.asarray(
        [[t[1] for t in system.cfg.idxarr_vec[layerind]] for layerind in layers]
    )  # device_put(jnp.asarray([[t[1] for t in system.cfg.idxarr_vec[layerind]] for layerind in layers]), device=ggpeps.PREFERRED_DEVICE)
    nlinks = system.cfg.lattice.nlinks
    gamma_in_sys_mod_vec = jnp.asarray(
        system.gamma_in_sys_mod_vec
    )  # device_put(jnp.asarray(system.gamma_in_sys_mod_vec), device=ggpeps.PREFERRED_DEVICE)
    diff_d_inv_gamma_inv_vec = jnp.asarray(
        [system.wi_gamma_in_mod_vec[layerind].inv() for layerind in layers]
    )  # device_put(jnp.asarray([system.wi_gamma_in_mod_vec[layerind].inv() for layerind in layers]), device=ggpeps.PREFERRED_DEVICE)
    mat_d_mod_inv_vec = jnp.asarray(
        system.mat_d_mod_inv_vec
    )  # device_put(jnp.asarray(system.mat_d_mod_inv_vec), device=ggpeps.PREFERRED_DEVICE)

    # get saved intermediate results from electric energy calculation
    intermediate = system._electric_energy_intermediate_vals
    covmat_out_virt_vec = jnp.asarray(
        intermediate.covmat_out_virt_vec
    )  # device_put(jnp.asarray(intermediate.covmat_out_virt_vec), device=ggpeps.PREFERRED_DEVICE)
    norm_mod_vec = jnp.asarray(
        intermediate.norm_mod_vec
    )  # device_put(jnp.asarray(intermediate.norm_mod_vec), device=ggpeps.PREFERRED_DEVICE)
    lognorm_default_vec = jnp.asarray(
        intermediate.lognorm_default_vec
    )  # device_put(jnp.asarray(intermediate.lognorm_default_vec), device=ggpeps.PREFERRED_DEVICE)
    pfaffian_vec = jnp.asarray(
        intermediate.pfaffian_vec
    )  # device_put(jnp.asarray(intermediate.pfaffian_vec), device=ggpeps.PREFERRED_DEVICE)

    # these depend on the symbol
    deriv_gamma_maj_sys_vec = jnp.asarray(
        [
            [
                system.gamma_maj_sys_deriv_vec(symbol)[layerind]
                for symbol in system.symbolvec
            ]
            for layerind in layers
        ]
    )  # device_put(, device=ggpeps.PREFERRED_DEVICE)
    trace_def_vec = jnp.asarray(
        [
            [
                system.compute_grad_over_norm(symbol, layerind)
                for symbol in system.symbolvec
            ]
            for layerind in layers
        ]
    ...
    dest_jax = batch_calculate_el_grads_all_layers(
        ...
        diff_d_inv_gamma_inv_vec,
        covmat_out_virt_vec,
        norm_mod_vec,
        lognorm_default_vec,
        deriv_gamma_maj_sys_vec,
        mat_d_mod_inv_vec,
        trace_def_vec,
    )
...
def deriv_pfarr_jax(covmat_out_virt, d_covmat_out_virt, pfaval, prefactor, ind):
    res = prefactor * derivative_pfaffian_jax(
        pfaval, covmat_out_virt[jnp.ix_(ind, ind)], d_covmat_out_virt[jnp.ix_(ind, ind)]
    )
    return res
...
def gamma_in_sys_mod_jax(gamma_in_sys, single_link_offset):
    ...
    return jax.lax.slice(
        gamma_in_sys, (single_link_offset, single_link_offset), (N, N)
    )  # TODO: should this be dynamic_slice?
...
def compute_el_grad_onelayer_onesymbol(
    ...
    d_el_energy = jnp.real(overall_factor * jnp.sum(deriv_pfarr)) * jnp.exp(
        norm_mod - lognorm_default
    )

    # Summand with derivative of norms
    trace_mod = compute_grad_over_norm_jit(
        gamma_in_sys_mod, diff_d_inv_gamma_inv, d_mat_d, mat_d_mod_inv
    )
...
batch_calculate_el_grads_all_symbols = jax.vmap(
    compute_el_grad_onelayer_onesymbol, in_axes=in_axes_symbols
)  # for one layer, all symbols
in_axes_layers = (0, 0, 0, None, None, 0, 0, 0, 0, None, 0, 0, 0, 0, 0, 0, 0, 0)
batch_calculate_el_grads_all_layers = jax.vmap(
    batch_calculate_el_grads_all_symbols, in_axes=in_axes_layers
)  # for all layers, all symbols
