...
def build_unique_el_terms(
    idx_vec: Sequence[IdxGroup], coeffs_vec: Sequence[CoeffsGroup]
) -> tuple[IdxGroup, CoeffsVec]:
    ...
    so evaluating the Pfaffians per group element repeats about half the work. From the raw
    per-group-element structures this builds:
    ...
    Returns:
        tuple: (uniq_idx_vec, uniq_coeffs_vec)
    """
    num_ge = len(idx_vec)
    nlayer = len(idx_vec[0])
    nlinks = len(idx_vec[0][0])
    ...
    for lay in range(nlayer):
        ...
        for link in range(nlinks):
            ...
            for ge in range(num_ge):
                for bucket in idx_vec[ge][lay][link]:
                    if not bucket:
                        continue
            ...
            for ge in range(num_ge):
                ...
                for bucket, coeff_bucket in zip(idx_vec[ge][lay][link], coeffs_vec[ge][lay][link]):
                    if not bucket:
                        continue
...
class Config2DBase(ABC):
    ...
    def init_el_energy_terms(self) -> None:
        ...
        Implementations must build the per-group-element term structures and hand them to
        self.set_el_energy_terms(idx_vec, coeffs_vec, constants_vec), which keeps only the
        constants and the derived unique-basis (deduplicated) form the system evaluates.
        """
        raise NotImplementedError("Implement in subclass: must call set_el_energy_terms.")

    def set_el_energy_terms(
        self,
        idx_vec: Sequence[IdxGroup],
        coeffs_vec: Sequence[CoeffsGroup],
        constants_vec: Sequence[ConstantsGroup],
    ) -> None:
        """Digest the per-group-element electric-energy terms built by init_el_energy_terms.

        Only what evaluation needs is stored: the per-group-element constants and the unique-basis
        (deduplicated across group elements) index/coefficient structures. The raw per-group-element
        idx/coeffs structures are consumed here and not kept.

        Sets:
            - constants_vec[ge][layer][link]: constant term of the gauged projector
